<script setup>
    ...
    const startTimer = () => {
        intervalId = setInterval(() => {
            timer.value++ // INCREMENTATIONNNNNNNNNNNN
        }, 1000)
    }
    ...
    const formatTime = (seconds) => {
        ...
        return `${mins}:${secs.toString().padStart(2, '0')}` // Pour afficher le timer
    }

    const flipCard = (selectedCard) => {
        if (selectedCard.isFlipped || selectedCard.isMatched || flippedCards.value.length === 2) {
            return // Pour eviter que plus de 2 cartes soit juste retournés. NOTE : || c'est ou
        }
    ...
    const checkMatch = () => {
        ...
        if (card1.value === card2.value) {
            ...
            if (card.value.every(cardIndivididuelle => cardIndivididuelle.isMatched)) { // Verifie 1 par 1 les cartes
                stopTimer()
